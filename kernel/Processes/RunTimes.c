...
void* RtlpIsUtf8Process(char arg1) {
    ...
    if (*(wchar_t*)((uintptr_t)globals + 0x428) == 0xfde9 || 
        *(wchar_t*)((uintptr_t)globals + 0x468) == 0xfde9) {
        ...
        if (arg1 != 0) {
            if (*(wchar_t*)((uintptr_t)rcx + 0x34e) != 0xfde9) {
                return (void*)0; // Return 0 if not UTF-8
            } else {
                return (void*)1; // UTF-8 flag is set
            }
        } else if (*(wchar_t*)((uintptr_t)rcx + 0x34c) == 0xfde9) {
            return (void*)1; // Return 1 if UTF-8 flag is set
        }
    }
...
uint64_t CountUnicodeToUTF8(const wchar_t* utf16Src, int32_t utf16Length, uint32_t* utf8Size) {
    int64_t utf8Count = 0;
    const wchar_t* utf16End = utf16Src + (utf16Length >> 1);
...
uint64_t RtlUnicodeToUTF8N(char* utf8Dest, int32_t utf8MaxBytes, uint32_t* utf8BytesWritten, const wchar_t* utf16Src, char strictCheck) {
    ...
    const wchar_t* utf16SrcEnd = utf16Src + (strictCheck >> 1);
    int32_t utf8Count = 0;
...
int64_t RtlUnicodeToMultiByteN(void* dest, uint32_t destSize, uint32_t* bytesCopied, wchar_t* src, int32_t srcSize) {
    if (RtlpIsUtf8Process(0)) {
        ...
        if (bytesCopied == NULL) {
            ...
            bytesCopied = (uint32_t*)&tempBytesCopied;
        }
        if (srcSize == 0) {
            *bytesCopied = 0;
        } else {
    ...
    if (RtlpIsUtf8Process(0) == 0) {
        ...
        if (*(uint32_t*)((uintptr_t)globals + 0x434) == 0) {
            ...
            while (remaining != 0) {
                ...
                rsi++;
                wchar_t val = *(*(wchar_t**)((uintptr_t)globals + 0x450) + (ch << 1)) >> 8;
