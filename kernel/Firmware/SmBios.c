...
char* SmbiosGetString(SMBIOS_HEADER* Header, UINT8 Index) {
    ...
    return (*StringArea) ? StringArea : 0x00;
}

void RegisterDmiDevice(DMI_FIELD Field, string String);

UNUSED static void DmiRegisterFromSmBios(DMI_FIELD Field, string String){
    if(!String){
...
static void IterateSmbiosTables(uint8_t* TablePtr, uint8_t* End) {
    while (TablePtr < End) {
        ...
        if(Header->Type == SMBIOS_TYPE_FIRMWAE_INFORMATION){
        ...
        }else if(Header->Type == SMBIOS_TYPE_SYSTEM_INFORMATION){
