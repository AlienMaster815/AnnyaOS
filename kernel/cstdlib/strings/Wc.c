...
wchar_t* wcschr(wchar_t* str, wchar_t ch) {
    wchar_t current_char = *str;  // Load the first character
...
wchar_t* wcscpy(wchar_t* dest, const wchar_t* src) {
    wchar_t* dest_start = dest;  // Pointer to the beginning of dest to return at the end
    wchar_t* offset_ptr = (wchar_t*)(dest - src);  // Calculate the offset between dest and src

    // Copy characters from src to dest until the null terminator is reached
    do {
        wchar_t ch = *src;           // Read character from src
        *(wchar_t*)((uint64_t)offset_ptr + (uint64_t)src) = ch;    // Write character to dest based on offset
        src++;
...
int64_t wcscspn(const wchar_t* str1, const wchar_t* str2) {
    const wchar_t* start = str1;

    while (*str1 != 0) {
        const wchar_t* check = str2;
        while (*check != 0) {
            if (*str1 == *check) {
...
wchar_t* wcsrchr(wchar_t* arg1, wchar_t arg2) {
    wchar_t* r9 = arg1;  // Save the start of the string
    wchar_t i;
...
const wchar_t* _pwctype = PwcBuffer;
...
uint64_t _iswctype_l(wchar_t wchar, wchar_t charClass) {
    ...
    if (wchar >= 0x100) {
        ...
        result = (_pwctype[wchar] & (wchar_t)charClass);
    }
...
uint64_t wcstoul(wchar_t* arg1, int64_t* arg2, int32_t arg3) {
    ...
    return wcstoxlX(0x00, arg1, (wchar_t**)arg2, arg3, 1, 0);
}
...
int64_t wcsspn(wchar_t* arg1, wchar_t* arg2) {
    wchar_t i = *arg1;  // Initialize `i` with the first character of arg1
    wchar_t* rax = arg1;  // Set `rax` to the start of arg1
    
    // Loop through each character in arg1 until reaching null terminator
    while (i != 0) {
        wchar_t* r9_1 = arg2;  // Pointer to iterate through arg2
        wchar_t r11_1 = *arg2;  // First character of arg2
...
wchar_t* LouKeCharToWchar(const char* src) {
    if (src == 0x00) return 0x00; // Handle null input
    ...
    for (size_t i = 0; i < length; i++) {
        dest[i] = (wchar_t)src[i]; // Simple char-to-wchar conversion
    }

    dest[length] = L'\0'; // Null-terminate the wide character string
    return dest;
}