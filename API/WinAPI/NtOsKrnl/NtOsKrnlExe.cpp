...
NTSTATUS HeadlessDispatch(
    ...
    void* HeadlessGlobals = LouKeGetHeadlessGlobals();

    if((HeadlessGlobals != 0) && (*(uint64_t*)((string)HeadlessGlobals + 8) != 0)){
        return LouKeHdlspDispatch(    
            DispatchCommand, 
            CommandData, 
            Flags, 
            HeadlessLocal, 
    ...
    if((DispatchCommand == 1) || (DispatchCommand ==  0x15)){
        return STATUS_UNSUCCESSFUL;
    }

    if((DispatchCommand <= 0x10) && (TEST_BITD(0x13804, DispatchCommand))){
        if((HeadlessLocal == 0) || (size == 0)){
            return STATUS_INVALID_PARAMETER;
        }
        memset(HeadlessLocal, 0, *size);
    }
...
int64_t InterlockedPushListSList(
    ...
    uint64_t HighValue = InputList[1];
    uint64_t Result;
    bool Finished = false;

    do{
        Result = (HighValue & 0xF0);
        *(uint64_t*)OutputList = Result;
        if(0 == *(unsigned __int128*)InputList){
            Finished = true;
        }
        else {
            HighValue = HighQuad(InputList);
            Finished = false;
        }
...
//EmProviderRegiter
//EmProviderRegistryEntry
//EmpProviderRegister
//DifFindThreadContextData
//DifGetPluginPerDriverData
//DifGetPluginSimplePerfControl
...
//DbgWerCaptureLiveKernelDump
//DbgWerCaptureLiveKernelDump2
...
//CcSetLogDataThreshold
//CcSetParralelFlushFile
