...
LOUSTATUS ResetAhcPciController(PATA_HOST_DEVICE_OBJECT AtaHost);
...
static LOUSTATUS AhciStopFisReception(PATA_PORT_DEVICE_OBJECT AhciPort){
    ...
    Status = LouKeMmioWaitTillClear((PULONG)&Port->PxCMD, AHCI_PxCMD_FR, 10, 1000);
...
LOUSTATUS AhciGenericPortDeviceGetCommandStatus(PATA_PORT_DEVICE_OBJECT PortDevice, PATA_COMMAND_PACKET CommandPacket){
    ...
    for(SIZE i = 0; i < 10; i++){
        if(PrivateData->GenericPort->PxIS & (1UL << 30)){
            ...
            UINT8 TfdStatus = Tfd & UINT8_MAX;
            if(CommandPacket->CommandFlags & ATA_COMMAND_PACKET_FLAGS_EXT_CMD){
                CommandPacket->PacketEx.Status = TfdStatus;
                CommandPacket->PacketEx.Error = TfdError;
            }else{
                CommandPacket->Packet.Status = TfdStatus;
                CommandPacket->Packet.Error = TfdError; 
            }
            return STATUS_IO_DEVICE_ERROR;
        }
        Status = LouKeMmioWaitTillClear((PULONG)&PrivateData->GenericPort->PxCI, 1UL << CommandPrivateData->CommandSlot, 10, 500);
        if(Status == STATUS_SUCCESS){
            break;
        }
    }
...
//TODO: Allocate 1 Command Table Structure per Header 

static 
LOUSTATUS 
AhciGenericGetFreeCommandSlot(PAHCI_DRIVER_PRIVATE_DATA PrivateData, UINT8* Slot){
    for(UINT8 i = 0 ; i < 32; i++){
        if(!(PrivateData->CommandSlot & (1 << i))){
            PrivateData->CommandSlot |= (1 << i);
            *Slot = i;
            return STATUS_SUCCESS;
        }
    }
...
LOUSTATUS AhciGenericPortDeviceCleanupCommand(
    ...
    if(CommandPacket->CommandStatus != STATUS_SUCCESS){
        LouPrint("CommandStatus:%h\n", CommandPacket->CommandStatus);
        AhciHandleCommandErrorRecovery(PortDevice, CommandPacket);
    } 
    if(CommandPacket->CommandFlags & ATA_COMMAND_PACKET_FLAGS_DMA){
        return STATUS_SUCCESS;
    }
...
//LOUSTATUS AhciResetPortDevice(PATA_PORT_DEVICE_OBJECT PortDevice){

//    LouPrint("AhciResetPortDevice()\n");
//    while(1);
//}


static LOUSTATUS AhciStartPort(PATA_PORT_DEVICE_OBJECT PortDevice){
    PAHCI_DRIVER_PRIVATE_DATA PrivateData = (PAHCI_DRIVER_PRIVATE_DATA)PortDevice->PortPrivateData;

    AhciStartFisReception(PortDevice);

    if(
        (!PrivateData->GenericPort->PxSIG) ||
        (PrivateData->GenericPort->PxSIG == UINT32_MAX)
    ){
        AhciStopFisReception(PortDevice);
...
LOUSTATUS AhciPortResume(PATA_PORT_DEVICE_OBJECT PortDevice){
    ...
    return AhciStartPort(PortDevice);
}
...
LOUSTATUS AhciGenericPortDeviceStopPort(PATA_PORT_DEVICE_OBJECT PortDevice){
    AhciDeInitalizePort(PortDevice);
    return STATUS_SUCCESS;
}


LOUSTATUS AhciGenericHostReset(PATA_HOST_DEVICE_OBJECT AtaHost){
    ResetAhcPciController(AtaHost);
    return STATUS_SUCCESS;
}

LOUSTATUS AhciGenericHostStart(PATA_HOST_DEVICE_OBJECT AtaHost){
    AhciPciInitializeController(AtaHost);
    PciHalEnableBusMaster(AtaHost->PDEV);
    return STATUS_SUCCESS;
}

static ATA_HOST_OPERATIONS AhciGenericHostOperations = {
    .AtaHostDeviceReset = AhciGenericHostReset,
    .AtaHostDeviceStart = AhciGenericHostStart,
...
static ATA_PORT_OPERATIONS AhciGenericPortOperations{
    .AtaPortDevicePrepCommand = AhciGenericPortDevicePrepCommand,
    .AtaPortDeviceIssueCommand = AhciGenericPortDeviceIssueCommand,
    .AtaPortDeviceGetCommandStatus = AhciGenericPortDeviceGetCommandStatus,
...
static ATA_PORT_OPERATIONS AhciVt8251PortOperations{
    .AtaPortDevicePrepCommand = AhciGenericPortDevicePrepCommand,
    .AtaPortDeviceIssueCommand = AhciGenericPortDeviceIssueCommand,
    .AtaPortDeviceCleanupCommand = AhciGenericPortDeviceCleanupCommand,
    .AtaPortDeviceStart = AhciGenericPortDeviceStartPort,
...
static ATA_PORT_OPERATIONS AhciP5wdhPortOperations{
    .AtaPortDevicePrepCommand = AhciGenericPortDevicePrepCommand,
    .AtaPortDeviceIssueCommand = AhciGenericPortDeviceIssueCommand,
    .AtaPortDeviceCleanupCommand = AhciGenericPortDeviceCleanupCommand,
    .AtaPortDeviceStart = AhciGenericPortDeviceStartPort,
...
static ATA_PORT_OPERATIONS AhciAvnPortOperations{
    .AtaPortDevicePrepCommand = AhciGenericPortDevicePrepCommand,
    .AtaPortDeviceIssueCommand = AhciGenericPortDeviceIssueCommand,
    .AtaPortDeviceCleanupCommand = AhciGenericPortDeviceCleanupCommand,
    .AtaPortDeviceStart = AhciGenericPortDeviceStartPort,
...
static ATA_PORT_OPERATIONS AhciPmpRetySrStPortOperations{
    .AtaPortDevicePrepCommand = AhciGenericPortDevicePrepCommand,
    .AtaPortDeviceIssueCommand = AhciGenericPortDeviceIssueCommand,
    .AtaPortDeviceCleanupCommand = AhciGenericPortDeviceCleanupCommand,
    .AtaPortDeviceIsaPacketDevice = AhciPortDeviceIsaPacketDevice,
...
LOUSTATUS AhciStopCommandEngine(PATA_PORT_DEVICE_OBJECT AtaPort){
    ...
    Port->PxCMD = Tmp;
    Tmp = 0;
    Status = LouKeMmioWaitTillClear((PULONG)&Port->PxCMD, AHCI_PxCMD_CR, 10, 1000);
    if(Status != STATUS_SUCCESS){
        return Status;
    }
...
LOUSTATUS AhciStartCommandEngine(PATA_PORT_DEVICE_OBJECT AtaPort){
    PAHCI_DRIVER_PRIVATE_DATA PrivateData = (PAHCI_DRIVER_PRIVATE_DATA)LkdmAtaPortToPrivateData(AtaPort); 
    PAHCI_GENERIC_PORT Port = PrivateData->GenericPort;
    LOUSTATUS Status;
    ...
    Port->PxCMD = Command;
    Command = Port->PxCMD;
    Status = LouKeMmioWaitTillSet((PULONG)&Port->PxCMD, AHCI_PxCMD_CR, 10, 1000);
    if(Status != STATUS_SUCCESS){
        return Status;
    }
...
static void AhciRemapCheck(
    ...
    PrivateData->BoardInfo.AhciFlags |= AHCI_FLAG_NO_MSI;
}
...
static void AhciMarkExternalPort(PATA_PORT_DEVICE_OBJECT AtaPort){
...
static void AhciUpdateInitialLpmPolicy(PATA_PORT_DEVICE_OBJECT AtaPort){
...
static void AhciIntelPcs(PPCI_DEVICE_OBJECT PDEV, PAHCI_DRIVER_PRIVATE_DATA PrivateData){
    ...
    if(!(PrivateData->BoardInfo.AhciFlags & AHCI_FLAG_INTEL_PCS)){
        return;
    }
    ...
    if((Tmp & (PrivateData->PortMap & UINT16_MAX)) != (PrivateData->PortMap & UINT16_MAX)){
...
static void AhciEnableAhci(PAHCI_GENERIC_HOST_CONTROL Ghc){
    uint32_t Tmp;
    ...
    LouPrint("AHCI.SYS:WARNING:Unable To Enable Host Controller\n");
}

static LOUSTATUS ResetAhciHba(PATA_HOST_DEVICE_OBJECT AtaHost){
    ...
    Status = LouKeMmioWaitTillClear((PULONG)&Ghc->GlobalHostControl, 0x01, 10, 1000);
    if(Status != STATUS_SUCCESS){
        return Status;
    }
...
static void AhciClearPortPendingIrq(PATA_PORT_DEVICE_OBJECT AhciPort){
    PAHCI_DRIVER_PRIVATE_DATA PrivateData = (PAHCI_DRIVER_PRIVATE_DATA)LkdmAtaPortToPrivateData(AhciPort);
    PAHCI_GENERIC_PORT Port = PrivateData->GenericPort;
...
void AhciStartFisReception(PATA_PORT_DEVICE_OBJECT AhciPort){
    ...
    //Flush Command
    Command = Port->PxCMD;
    Status = LouKeMmioWaitTillSet((PULONG)&Port->PxCMD, AHCI_PxCMD_FR, 10, 1000);
    if(Status != STATUS_SUCCESS){
        return;
    }
...
void AhciInitializeController(PATA_HOST_DEVICE_OBJECT AtaHost){
    ...
    ForEachAtaPort(AtaHost, TmpPort, i){
        Status = AhciDeInitalizePort(TmpPort);
        if(Status != STATUS_SUCCESS){
            LouPrint("AHCI.SYS:WARNING:Unable To DeInit Port:%d\n", i);
        }
        AhciClearPortPendingIrq(TmpPort);
    }       
...
void AhciPciInitializeController(PATA_HOST_DEVICE_OBJECT AtaHost){
    ...
    if(AhciPrivate->BoardInfo.AhciFlags & AHCI_FLAG_MV_PATA){
        if(PciHalGetDeviceId(PDEV) == 0x6121){
            Mv = 2;
        }else{
            Mv = 4;
        }
        ...
        if(AtaHost->PortCount > Mv){
            PAHCI_DRIVER_PRIVATE_DATA MvPort = (PAHCI_DRIVER_PRIVATE_DATA)AtaHost->PortDevices[Mv].PortPrivateData;
...
LOUSTATUS AddAhciDevice(
    ...
    //TODO: save controller config and reset at the reset function    
    LOUSTATUS Status = STATUS_SUCCESS;
    ...
    if(
        (BoardID == AHCI_BOARD_MCP65_CHIP && PciHalGetRevisionId(PDEV) == 0xA1) || 
        (BoardID == AHCI_BOARD_MCP65_CHIP && PciHalGetRevisionId(PDEV) == 0xA2) 
    ){
        PrivateAhciData->BoardInfo.AhciFlags |= AHCI_FLAG_NO_MSI;
    }
    //SB800 doesn't need to conform to the INTERNAL SERR quirk
    if(BoardID == AHCI_BOARD_SB700_CHIP && PciHalGetRevisionId(PDEV) >= 0x40){
        PrivateAhciData->BoardInfo.AhciFlags &= ~(AHCI_FLAG_IGN_INTERNAL_SERR);
    }
    ...
    if(AHCI_SUPPORTS_SNCQ(Ghc->Capabilities)){
        //PrivateAhciData->BoardInfo.AtaFlags |= ATA_FLAG_NCQ;
        ...
        if(!(PrivateAhciData->BoardInfo.AhciFlags & AHCI_FLAG_NO_FPDMA_AA)){
            //PrivateAhciData->BoardInfo.AtaFlags |= ATA_FLAG_FPDMA_AA;
        }

        //Finally all systems with NCQ have a Auxil field
        //PrivateAhciData->BoardInfo.AtaFlags |= ATA_FLAG_FPDMA_AUXILERY;
    }

    if(AHCI_SUPPORTS_S64A(Ghc->Capabilities)){
        if(PrivateAhciData->BoardInfo.AhciFlags & AHCI_FLAG_43BIT_DMA_ONLY){
            PrivateAhciData->DmaDevice.AllocatorData.DmaLimit = 43;        
        }else{
            PrivateAhciData->DmaDevice.AllocatorData.DmaLimit = 64;        
        }
    }else {
        PrivateAhciData->BoardInfo.AhciFlags |= AHCI_FLAG_32BIT_ONLY;
    ...
    if(AhciHasBrokenSystemPowerOff(PDEV)){
        PrivateAhciData->BoardInfo.AhciFlags |= AHCI_FLAG_NO_POWEROFF_SPINDOWN;
    ...
    if(AhciHasBrokenSuspend(PDEV)){
        PrivateAhciData->BoardInfo.AhciFlags |= AHCI_FLAG_NO_SUSPEND;
        LouPrint("AHCI.SYS:BIOS Update Required For Suspend And Resume\n");
    }
    
    if(AhciHasBrokenOnline(PDEV)){
        PrivateAhciData->BoardInfo.AhciFlags |= AHCI_SRST_TOUT_IS_OFFLINE;
    ...
    Status = AtaCoreRegisterAtaHostDevice(AtaHost);    
    if(Status != STATUS_SUCCESS){
        LouPrint("AHCI.SYS:AddAtaDevice() Could Not Register AHCI Host Device\n");
        while(1);
    }
