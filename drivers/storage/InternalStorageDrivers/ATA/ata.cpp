...
//Driver has been retired see 2.0 in /ATA/Ata2.cpp
...
void PATA::ReadPatapiCommand(uint16_t drive,bool Master, uint32_t Sector_Num, void* Buffer, int BufferSize){
 uint32_t sectorCount = (BufferSize + 2047) / 2048;
...
void PATA::Read28PATAPI(uint16_t drive,bool Master, uint32_t Sector_Num, void* Buffer, int BufferSize){
    ...
    //Read the data
    ReadPatapiCommand(drive, Master, Sector_Num, Buffer,BufferSize);
    
    //Check For No Data if no data is there double check that there is actually no data
    bool DataRecived = false;;
    uint64_t* Buffer64 = (uint64_t*)Buffer;
    for(uint32_t i = 0 ; i < (uint32_t)(BufferSize/8); i++ ){
        if(Buffer64[i] != 0){
            DataRecived = true;
        }
    }
    //If there is no data re read and return the new value
    if(!DataRecived){
        ReadPatapiCommand(drive, Master, Sector_Num, Buffer, BufferSize);
    }
